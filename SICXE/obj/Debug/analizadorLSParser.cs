...
public partial class analizadorLSParser : Parser {
        List<string> codObj;//Lista para el manejo del cp y errores
        List<List<string>> tabSim;//lista para la tabla de simbolos
        string l = "";//variable global
        public const int
	...
	[RuleVersion(0)]
	public ProgramaContext programa(ref List<string> cod, ref List<List<string>> tab) {
            this.codObj = cod;
            this.tabSim = tab;
	...
	public TipodirectivaContext tipodirectiva() {
		...
		try {
			...
			case 4:
				...
				State = 74; Match(WORD);
				State = 75; opvalor();
                            this.l = _localctx.start.InputStream.ToString();
                            aumentaCp("W");
				}
				break;
	...
	public EtiquetaContext etiqueta() {
		...
		try {
			...
			{
			State = 111; Match(M);
            agregaSim(_localctx.start.Text,_localctx.start.InputStream.ToString());
			}
		}
		catch (RecognitionException re) {
                this.codObj[1] = "Error:Instruccion no existe";
	...
	public OpdirectivaContext opdirectiva() {
        ...
        //Metodo que agrega un nuevo dato a la lista que corresponde a la tabla de simbolos con su respectiva direccion
        public void agregaSim(string s,string input) {
            if (!tabSim[0].Contains(s) && !(input.Contains("START")))
            {
                this.tabSim[0].Add(s);//simbolo
                this.tabSim[1].Add(this.codObj[0]);//direccion
            }
            else if (!(input.Contains("START")))
            {
                this.codObj[1] = "Error:Simbolo Duplicado";//Se carga el error
            }
        }

        //Metodo empleado para el calculo del cp
        //recibe un valor de tipo string para diferenciar los casos (Formato 1..2.3..4, directivas )
        public void aumentaCp(string f) {
            int cp = 0;
            switch (f) {
                case "1":
                    cp = Convert.ToInt32(this.codObj[0], 16);
                    cp += 1;
                    this.codObj[0] = cp.ToString("X");
                    break;
                case "2":
                    cp = Convert.ToInt32(this.codObj[0], 16);
                    cp += 2;
                ...
                case "C":
                    ...
                    break;
                case "RW":
                    string[] lis = this.l.Split(' ');
                    string valu = lis[lis.Length - 1];
                    if (valu.Contains("H"))
                    {
                        valu = valu.Replace("H", "");
                ...
                case "RB":
                    ...
                    break;
                case "W":
                    string[] t = this.l.Split(' ');
                    string tem = t[t.Length - 1];
                    cp = Convert.ToInt32(this.codObj[0], 16);
                    cp += 3;
                    this.codObj[0] = cp.ToString("X");
